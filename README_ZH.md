...
2. **终端路由** — 应用从 `UserDefaults` 读取 `terminal` 键值（通过 `defaults write com.go2shell.MyGo2Shell terminal "name"` 设置）。原始值经过清洗，剥离字母数字、空格和连字符以外的所有字符——这是为了防止 AppleScript 注入，因为终端名称会被插值到脚本字符串中。清洗结果若为空则回退到 `Terminal`。清洗后的名称通过大小写无关匹配分派到内置处理器：iTerm / iTerm2 始终新开一个窗口（运行中走 `create window with default profile`，冷启动直接复用自动生成的初始窗口）；Ghostty 运行中走 `perform action "new_window"` 对 focused terminal 发动作——与 Cmd+N 走同一条 keybind notification 路径，绕开 scripting 路径下 `tabbingMode = .preferred` 导致 `new window with configuration` 被 macOS 自动合入 tab 组的问题——再通过 `perform action "text:..."` 把 `cd <path> && clear` + 回车投给新 terminal；Ghostty 冷启动回退到 `new window with configuration`（此时无已有 tab 组可合并，安全）；Warp 通过 AppleScript `do shell script "open -a Warp <path>"` 调用原生目录参数；其余终端落到通用 `do script` AppleScript 处理器。每个处理器都区分「应用已在运行」的快速路径与「冷启动后轮询 `count of windows > 0`」的慢速路径，所有 `cd` 都会追加 `&& clear` 以呈现干净的提示符。如果配置的终端在 `/Applications/`、`/System/Applications/`、`/System/Applications/Utilities/` 或 `~/Applications/` 中均未找到，则自动回退到 Terminal.app。
...
- **处理器特化** — 每个处理器针对目标终端优化：iTerm Handler 始终新开一个窗口（运行中走 `create window with default profile`，冷启动直接复用自动生成的初始窗口）；Ghostty Handler 在运行中的快速路径对 focused terminal 发 `perform action "new_window"`（与 Cmd+N 同一条 runloop 路径，绕开 scripting 路径下 `tabbingMode = .preferred` 触发的自动 tab 合并），再通过 `perform action "text:..."` 把 `cd <path> && clear` + 回车发到新 terminal；冷启动走 `new window with configuration`（此时无已有 tab 组可合，安全）；Warp Handler 通过 AppleScript `do shell script "open -a Warp <path>"` 调用（Warp 直接接受目录参数）；Generic Handler 使用通用的 `do script` AppleScript 接口兼容所有支持脚本的终端。每个处理器都区分「应用已运行」的快速路径与「冷启动后等待首个窗口出现」的慢速路径。
